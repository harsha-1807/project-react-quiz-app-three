...
const Second = () => {
  ...
  return (
    <div style={{backgroundColor:"#3B2F7F",width:"100vw",height:"100vh",display:"flex",justifyContent:"center",alignItems:"center"}}>
        {/* <div></div> */}
        {n===5 ? (<Third/>):(
        ...
        <div className='grid' >
          {quizQuestions.options.map((option, i) => (
            <button
              ...
              style={{backgroundColor:"#1795EB",borderRadius:"20vh",width:"25vw"}}
            >
              {option}
            </button>
          ))}
        ...
        <div className='btns'>
          <button className='btn' onClick={() => setN((prev) => Math.max(prev - 1, 0))} style={{backgroundColor:"#336F9C"}}>
            P R E V I O U S
          </button>
          <button className='btn' onClick={() => setN((prev) => (prev < 9 ? prev + 1 : 0))} style={{backgroundColor:"#007100"}}>
            N E X T
          </button>
          <button className='btn' onClick={() => window.confirm('Are you sure you want to quit?')} style={{backgroundColor:"red"}}>
            Q U I T
          </button>
          <Link to='/3'>
            <button className='btn' style={{backgroundColor:"#336F9C"}}>F I N I S H</button>
          </Link>
        </div>
      </div>
)}</div>
